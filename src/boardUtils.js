...
const markBoard = (board, position, mark) => {
  ...
  if (mark !== 'X' && mark !== 'O') {
    return { type: 'error', value: 'wrong mark. Only "X" or "O" allowed' };
  }
  if (board[position] === 'X' && board[position] === 'O') {
    return { type: 'error', value: 'position already taken' };
  }
  board[position] = mark;
  return { type: 'success', value: board };
};
...
module.exports = { genBoard, markBoard, check_winner };
